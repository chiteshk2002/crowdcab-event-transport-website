...
def congestion_cell(lat, lng, size=0.01):
    return (math.floor(lat / size), math.floor(lng / size))


def build_congestion_index(congestion_points):
    index = defaultdict(list)
    for point in congestion_points:
        p_lat = num(point.get('latitude'), None)
        p_lng = num(point.get('longitude'), None)
        if p_lat is not None and p_lng is not None:
            index[congestion_cell(p_lat, p_lng)].append((p_lat, p_lng))
    return index


def nearby_congestion_count(lat, lng, congestion_points, radius_km=0.25):
    count = 0
    if isinstance(congestion_points, dict):
        base_lat, base_lng = congestion_cell(lat, lng)
        points = []
        for d_lat in [-1, 0, 1]:
            for d_lng in [-1, 0, 1]:
                points.extend(congestion_points.get((base_lat + d_lat, base_lng + d_lng), []))
    else:
        points = [
            (num(point.get('latitude'), None), num(point.get('longitude'), None))
            for point in congestion_points
        ]
...
def pickup_candidates_for_venue(venue_id):
    ...
    csv_rows = [
        row for row in read_csv('venue_pickup_points.csv')
        if row.get('venue_id') == venue_id and str(row.get('candidate_pickup_point', '1')) in ['1', 'true', 'True']
    ]
    if csv_rows:
        return csv_rows
    ...
    if venue_id != 'suncorp_stadium':
        print(f'[CrowdCab] warning: no pickup rows found for venue_id={venue_id}', file=sys.stderr)
        return []
...
def has_user_friendly_pickup_name(row):
    """Reject raw OSM placeholders like 'Unnamed Service Road' for user-facing recommendations."""
    text = ' '.join(
        str(row.get(key) or '').lower()
        for key in ['label', 'pickup_point_label', 'street', 'pickup_point_id']
    )
    return 'unnamed service' not in text and 'unnamed road' not in text and 'unnamed residential' not in text


def candidate_raw_rows(origin_lat, origin_lng, congestion_points, max_walk_km=DEFAULT_MAX_WALK_KM, venue_id='suncorp_stadium', search_radius_km=DEFAULT_SEARCH_RADIUS_KM, origin_is_user=False):
    candidates = pickup_candidates_for_venue(venue_id)
    named_candidates = [row for row in candidates if has_user_friendly_pickup_name(row)]
    if len(named_candidates) >= 8:
    ...
    for row in candidates:
        ...
        if origin_is_user or distance_km is None:
...
def recommend_pickups(venue_id='suncorp_stadium', user_lat=None, user_lng=None, preferences=None):
    ...
    origin_is_user = user_lat is not None and user_lng is not None

    congestion_points = build_congestion_index(read_csv('congestion_points.csv'))
