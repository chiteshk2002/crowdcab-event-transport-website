...
(function(){
  ...
  if(!C) return;
  const {qs, postJSON, customerLabelCrowd, safeText} = C;

  function activeTripStore(trip){
    try{
      const venueId = trip?.scores?.venue_id || trip?.venue_id || window.CrowdCabMap?.getSelectedVenueId?.();
      if(venueId) localStorage.setItem('crowdcab_selected_venue', venueId);
      localStorage.setItem('crowdcab_active_trip', JSON.stringify(trip));
    }catch(e){}
  }

  function finalDestinationValue(){
    const raw = (qs('#destinationInput')?.value || '').trim() || C.getStoredDestination?.() || '';
    const check = C.destinationValidation ? C.destinationValidation(raw) : {valid:!!raw, value:raw};
    return check.valid ? check.value : 'Destination not set';
  }
  ...
  function finalTripFromSelected(){
    ...
    return {
      ...
      venue_name: selectedVenue?.name || window.CrowdCabMap?.getSelectedVenueName?.() || 'Event venue',
  ...
  async function confirmSelectedPickup(){
    ...
    toast.innerHTML = `<strong>Pickup confirmed</strong><span>${safeText(savedTrip.pickup)} - ${safeText(savedTrip.walk_min)} min walk</span><div class="toast-actions"><a href="/guidance">Start GPS guidance</a><a href="/my-trips">View trip</a></div>`;
