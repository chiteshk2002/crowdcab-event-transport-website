...
(function(){
  ...
  function dashboardTable(table, title='Dataset records'){
    ...
    return `<section class="dashboard-table-card">
      ...
      <div class="dashboard-table-scroll"><table class="data-table"><thead><tr>${table.columns.map(c=>`<th>${safeText(String(c).replaceAll('_',' '))}</th>`).join('')}</tr></thead><tbody>
  ...
  function renderBookingsDash(root,d){
    ...
    root.innerHTML = `${dashKpiPanel(d.kpis)}
      <section class="booking-control-layout">
        <article class="dashboard-hero-card booking-hero-card"><span class="panel-label">Booking control</span><h2>Where booking requests are coming from</h2><p>Channel behaviour for event-day pickup guidance.</p><div class="channel-stack">${channelRows.map(r=>`<div><span>${safeText(r.name)}</span><b>${fmt(r.value)}</b></div>`).join('')}</div></article>
  ...
  function renderCabDash(root,d){
    ...
    root.innerHTML = `${dashKpiPanel(d.kpis)}<section class="fleet-ops-layout"><article class="fleet-status-panel"><span class="panel-label">Dispatcher view</span><h2>Fleet readiness by ETA</h2>${companyEta.slice(0,6).map(c=>`<div class="fleet-eta-row"><span>${safeText(c.company)}</span><em><i style="width:${Math.max(12,100-(c.eta*8))}%"></i></em><b>${c.eta.toFixed(1)} min</b></div>`).join('')}</article><article class="dashboard-plot-card"><div id="cabEtaBar" class="plot"></div></article><article class="dashboard-plot-card compact"><div id="cabStatusDonut" class="plot"></div></article></section>${dashboardTable(d.table,'Cab allocation dataset')}`;
  ...
  async function initSystem(){
    ...
    root.innerHTML = `<div class="system-overview-grid"><div class="system-flow-card"><h2>Data pipeline</h2><div class="flow-line">${data.flow.map(x=>`<span>${safeText(x)}</span>`).join('')}</div></div><div class="system-role-card"><h2>Role access</h2>${data.roles.map(r=>`<span class="role-chip">${safeText(r)}</span>`).join('')}</div></div><h2 class="console-heading">API endpoints</h2><div class="endpoint-grid">${data.endpoints.map(e=>`<div class="endpoint-card"><code>${safeText(e.path)}</code><span>${safeText(e.purpose)}</span></div>`).join('')}</div><h2 class="console-heading">Datasets</h2><div class="system-files dataset-grid">${data.files.map(f=>`<div class="file-card"><strong>${safeText(f.file)}</strong><span>${fmt(f.rows)} rows</span><code>${(f.columns || []).map(c=>safeText(c)).join(', ')}</code></div>`).join('')}</div>`;
  }
