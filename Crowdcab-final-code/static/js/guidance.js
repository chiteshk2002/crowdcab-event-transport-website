...
(function(){
  ...
  if(!C) return;
  const {qs, qsa, getJSON, customerLabelCrowd, safeText} = C;
  ...
  function tripVenueId(trip){
    return trip?.scores?.venue_id || trip?.venue_id || localStorage.getItem(VENUE_STORAGE_KEY) || 'suncorp_stadium';
  }
  ...
  async function fetchWalkingRoute(start, end){
    const url = `/api/walking-route?start_lat=${encodeURIComponent(start[0])}&start_lng=${encodeURIComponent(start[1])}&end_lat=${encodeURIComponent(end[0])}&end_lng=${encodeURIComponent(end[1])}`;
    const response = await fetch(url, {cache:'no-store'});
    if(!response.ok) throw new Error(`Route ${response.status}`);
    const data = await response.json();
    const coords = data.coords;
    if(!coords?.length) throw new Error('No walking route geometry');
    return {coords, steps: data.steps || []};
  }
  ...
  function scoreBadge(label, value){
    ...
    return `<span class="${band}"><b>${score}</b>${safeText(label)}</span>`;
  }
  ...
  function liveNote(pickup){
    ...
    return note || speed ? `<p class="live-traffic-note">${safeText(note)}${speed}</p>` : '';
  }
  ...
  function saveSelectedPickup(pickup){
    try{
      const venueId = pickup?.scores?.venue_id || pickup?.venue_id || guidance.recommendations?.venue_id;
      if(venueId) localStorage.setItem(VENUE_STORAGE_KEY, venueId);
      localStorage.setItem('crowdcab_active_trip', JSON.stringify(pickup));
    }catch(e){}
  }
  ...
  function renderSteps(routeSteps=null){
    ...
    qs('#gpsSteps').innerHTML = guidance.steps.map((s,i)=>`<li class="${i===0?'active':''}" data-step="${i}"><strong>${safeText(s.title)}</strong><span>${safeText(s.detail)}</span><em>${safeText(s.distance)}</em></li>`).join('');
  ...
  async function renderMapRoute(){
    ...
    guidance.pickupMarker = L.marker(end, {icon:gpsPickupIcon(), zIndexOffset:900}).addTo(guidance.map).bindPopup(`Pickup: ${safeText(selected.pickup)}`).openPopup();
  ...
  function renderRecommendationPanel(){
    ...
    panel.innerHTML = `
      ...
      ${mismatch ? `<div class="better-pickup-banner"><strong>Better pickup available: ${safeText(best.label)}</strong><button data-switch-pickup="${safeText(best.pickup_point_id)}">${safeText(switchText)}</button></div>` : ''}
      <div class="recommendation-best selected-guidance-pickup">
        <small>Selected pickup</small>
        <strong>${safeText(selected.pickup)}</strong>
        <p>${safeText(selected.reason || 'This is the pickup currently used for your walking route.')}</p>
        ${liveNote(selected)}
