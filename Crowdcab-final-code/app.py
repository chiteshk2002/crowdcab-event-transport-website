...
import csv, os, math, json, sqlite3, random, smtplib, re, secrets, time
...
app = Flask(__name__)
app.secret_key = os.getenv('SECRET_KEY', 'crowdcab-demo-secret-change-for-production')
...
SMTP_TIMEOUT_SECONDS = float(os.getenv('SMTP_TIMEOUT_SECONDS', '4'))
INTERNAL_ACCESS_PASSWORD = os.getenv('INTERNAL_ACCESS_PASSWORD', '').strip()
OTP_RATE_LIMIT_WINDOW_SECONDS = int(os.getenv('OTP_RATE_LIMIT_WINDOW_SECONDS', '3600'))
OTP_RATE_LIMIT_EMAIL_MAX = int(os.getenv('OTP_RATE_LIMIT_EMAIL_MAX', '5'))
OTP_RATE_LIMIT_IP_MAX = int(os.getenv('OTP_RATE_LIMIT_IP_MAX', '30'))
OTP_RATE_LIMITS = defaultdict(list)
WALKING_ROUTE_CACHE = {}
WALKING_ROUTE_CACHE_SECONDS = int(os.getenv('WALKING_ROUTE_CACHE_SECONDS', '900'))
    ...
    def wrapper(*args, **kwargs):
        if not session.get('email'):
            if request.path.startswith('/api/'):
                return jsonify({'error': 'Log in before continuing.'}), 401
...
def valid_internal_password(password):
    return bool(INTERNAL_ACCESS_PASSWORD) and (password or '') == INTERNAL_ACCESS_PASSWORD
...
def rate_limit_key(kind, value):
    return f'{kind}:{value}'

def prune_rate_limit(key, now=None):
    now = now or time.time()
    cutoff = now - OTP_RATE_LIMIT_WINDOW_SECONDS
    OTP_RATE_LIMITS[key] = [stamp for stamp in OTP_RATE_LIMITS[key] if stamp >= cutoff]
    return OTP_RATE_LIMITS[key]

def allow_otp_request(email):
    now = time.time()
    ip = request.headers.get('X-Forwarded-For', request.remote_addr or 'unknown').split(',')[0].strip()
    email_key = rate_limit_key('email', email)
    ip_key = rate_limit_key('ip', ip)
    email_hits = prune_rate_limit(email_key, now)
    ip_hits = prune_rate_limit(ip_key, now)
    if len(email_hits) >= OTP_RATE_LIMIT_EMAIL_MAX or len(ip_hits) >= OTP_RATE_LIMIT_IP_MAX:
        return False
...
def start_customer_otp(email, resend=False):
    if not allow_otp_request(email):
        flash('Too many login code requests. Please wait before trying again.')
        return False
    otp = generate_otp()
...
def verify_customer_otp(otp):
    ...
    if expired:
        ...
        return False
    attempts = int(num(pending.get('attempts'), 0))
    ...
    if otp != expected:
        ...
        if remaining:
            flash(f'That code did not match. {remaining} attempt(s) remaining.')
        else:
            session.pop('pending_auth', None)
            flash('Too many incorrect attempts. Please request a new code.')
        return False
...
def known_venue_ids():
    return {venue.get('venue_id') for venue in get_venues() if venue.get('venue_id')}

def valid_venue_id(venue_id):
    return (venue_id or 'suncorp_stadium') in known_venue_ids()

def require_valid_venue_id(venue_id=None):
    venue_id = venue_id or request.args.get('venue_id') or 'suncorp_stadium'
    if not valid_venue_id(venue_id):
        return None, None, (jsonify({'error': f'Unknown venue_id: {venue_id}'}), 400)
    venue = get_venue(venue_id)
    session['selected_venue_id'] = venue_id
    return venue_id, venue, None
...
def plausible_lat_lng(lat, lng):
    return lat is not None and lng is not None and -90 <= lat <= 90 and -180 <= lng <= 180

def brisbane_lat_lng(lat, lng):
    return lat is not None and lng is not None and -28.5 <= lat <= -26.5 and 152.0 <= lng <= 154.0
...
def pickup_point_summary_rows(bookings=None, allocations=None):
    ...
    for booking in bookings:
        zone = booking.get('confirmed_pickup_label') or booking.get('pickup_location_name') or 'Unknown'
...
def coordinates_from_bookings():
    ...
    for r in get_bookings():
        zone = r.get('confirmed_pickup_label') or r.get('pickup_location_name')
...
def pickup_recommendations(venue_id='suncorp_stadium'):
    ...
    for idx, r in enumerate(summary):
        ...
        if lat is None or lng is None:
            ...
            lat = venue_lat + math.sin(angle) * 0.0048
            lng = venue_lng + math.cos(angle) * 0.0056
        dist = km_between(venue_lat, venue_lng, lat, lng)
...
def get_summary():
    ...
    pickup_counts = Counter((r.get('confirmed_pickup_label') or r.get('pickup_location_name') or 'Unknown') for r in bookings)
...
def dashboard_payload(kind, venue_id='suncorp_stadium'):
    bookings = get_bookings()
    allocations = get_allocations()
    ...
    def pickup_name(row):
        return row.get('confirmed_pickup_label') or row.get('pickup_location_name') or 'Unknown'

    def booking_channel_label(value):
        cleaned = (value or '').strip()
        if cleaned.lower() in ['app', 'crowdcab app']:
            return 'CrowdCab Web'
        return cleaned or 'CrowdCab Web'

    if kind == 'bookings':
        booking_rows = []
        for row in bookings:
            cleaned = dict(row)
            cleaned['pickup_location_name'] = pickup_name(row)
            cleaned['booking_channel'] = booking_channel_label(row.get('booking_channel'))
            booking_rows.append(cleaned)
        by_channel = Counter(r.get('booking_channel', 'CrowdCab Web') for r in booking_rows)
        by_pickup = Counter(r.get('pickup_location_name', 'Unknown') for r in booking_rows)
        return {
            'summary': summary,
            'kpis': [
            ...
            'table': table(booking_rows, ['booking_id','pickup_location_name','dropoff_suburb','number_of_people','booking_channel'], 38)
        }

    if kind == 'pickups':
        recs = pickup_recommendations(venue_id)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if action == 'verify_otp':
            ...
            internal_password = request.form.get('internal_password') or ''
            email_error = email_validation_error(email)
            if email_error:
                flash(email_error)
            elif email in STAFF_USERS:
                if not valid_internal_password(internal_password):
                    flash('Internal access requires the assigned staff password.')
                    return render_template('login.html', active='login', pending_auth=None, attempted_email=email)
...
def api_map_feed():
    ensure_operational_tables()
    venue_id, venue, error = require_valid_venue_id()
    if error:
        return error
...
def api_map_preview():
    ensure_operational_tables()
    requested = request.args.get('venue_id') or session.get('selected_venue_id') or 'suncorp_stadium'
    if not valid_venue_id(requested):
        requested = 'suncorp_stadium'
    venue = get_venue(requested)
    venue_anchor = venue_map_anchor(venue)
    scored = recommend_pickups(
        venue_id=requested,
        preferences={
...
def api_recommend_pickups():
    ...
    venue_radius_km = num(venue.get('default_radius_km'), DEFAULT_MAX_WALK_KM)
    max_walk_km = num(request.args.get('max_walk_km'), DEFAULT_MAX_WALK_KM)
    max_walk_km = min(max(max_walk_km, 0.2), max(venue_radius_km, 0.2))
    ...
    return jsonify({
        ...
        'accessibility_required': accessibility_required,
        'pickup_filter': {
            'focus_area': venue_anchor.get('name') or venue_id.replace('_', ' ').title(),
            'max_walk_km': max_walk_km,
...
def api_realtime_traffic():
    ...
    if error:
        return error
    return jsonify(realtime_status(venue_map_anchor(venue)))

@app.route('/api/walking-route')
@login_required
def api_walking_route():
    start_lat = num(request.args.get('start_lat'), None)
    start_lng = num(request.args.get('start_lng'), None)
    end_lat = num(request.args.get('end_lat'), None)
    end_lng = num(request.args.get('end_lng'), None)
    if not all([
        plausible_lat_lng(start_lat, start_lng),
        plausible_lat_lng(end_lat, end_lng),
        brisbane_lat_lng(start_lat, start_lng),
        brisbane_lat_lng(end_lat, end_lng),
    ]):
        return jsonify({'error': 'Route coordinates are outside the supported Brisbane area.'}), 400
    cache_key = tuple(round(value, 5) for value in [start_lat, start_lng, end_lat, end_lng])
    cached = WALKING_ROUTE_CACHE.get(cache_key)
    now = time.time()
    if cached and now - cached['time'] <= WALKING_ROUTE_CACHE_SECONDS:
    ...
    for _ in range(2):
        try:
            ...
            if not coords:
                raise ValueError('No walking route geometry')
            payload = {
                'coords': [[lat, lng] for lng, lat in coords],
                'steps': ((route.get('legs') or [{}])[0]).get('steps') or [],
                'source': 'osrm_proxy',
            }
            WALKING_ROUTE_CACHE[cache_key] = {'time': now, 'data': payload}
            return jsonify(payload)
        except Exception as exc:
            last_error = exc
    app.logger.warning('Walking route proxy failed: %s', last_error)
    return jsonify({'error': 'Walking route unavailable'}), 502
...
def api_create_booking():
    ...
    venue_id = payload.get('venue_id') or 'suncorp_stadium'
    if not valid_venue_id(venue_id):
        return jsonify({'error': f'Unknown venue_id: {venue_id}'}), 400
    lat = num(payload.get('lat'), None)
    lng = num(payload.get('lng'), None)
    if not plausible_lat_lng(lat, lng) or not brisbane_lat_lng(lat, lng):
        return jsonify({'error': 'Pickup coordinates are outside the supported Brisbane area.'}), 400
    destination = str(payload.get('destination') or '').strip()
    if len(destination) > 160:
        return jsonify({'error': 'Destination is too long.'}), 400
...
@internal_required
def api_dash(kind):
    venue_id = request.args.get('venue_id') or session.get('selected_venue_id') or 'suncorp_stadium'
    if not valid_venue_id(venue_id):
        return jsonify({'error': f'Unknown venue_id: {venue_id}'}), 400
    return jsonify(clean(dashboard_payload(kind, venue_id)))
