...
REALTIME_FAILURE_CACHE_SECONDS = int(os.getenv("REALTIME_FAILURE_CACHE_SECONDS", "15"))
...
def _fresh(key):
    ...
    ttl = REALTIME_FAILURE_CACHE_SECONDS if _cache.get(key, {}).get("fallback_reason") else REALTIME_CACHE_SECONDS
    return ts and (_now() - ts) < ttl
